...
function NicheList({ handleRenderAddNiche, onEdit }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resp = await axios.get('http://3.110.194.148:5000/api/niche');
        setNiche(resp.data);
      } catch (error) {
  ...
  const handleDelete = async (id, nicheName) => {
    try {
      const result = await Swal.fire({
        title: `Are you sure to delete ${nicheName} ?`,
      ...
      if (result.isConfirmed) {
        await axios.delete(`http://3.110.194.148:5000/api/niche/${id}`);
        setNiche((prevNiche) => prevNiche.filter(item => item._id !== id));
        
        Swal.fire({
          title: "Deleted!",
          text: `Your ${nicheName} has been deleted.`,
          icon: "success"
        });
      }
    } catch (error) {
  ...
  // Filter the niche list based on the search term
  const filteredNiche = niche.filter((item) =>
    item.nicheName.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const body = filteredNiche.map((item) => {
    return {
      select: <input type="checkbox" onChange={(e) => handleSelectAll(e)} />,
      title: item.nicheName || "Unknown",
      ...
      action: (
        <>
          ...
          <button className='delete-btn' onClick={() => { handleDelete(item._id, item.nicheName); }}><RiDeleteBin6Line /></button>
        </>
      )
    };
  });
    ...
    <>
      <div className='d-flex align-items-center justify-content-between m-2 mb-2'>
        <div className='csv-heading'>
          <h3><IoMdArrowRoundBack className='me-1 mb-1' onClick={handleBack} />Niche</h3>
        </div>
        <div>
          <button className='clear-btn me-4' onClick={handleRenderAddNiche}>
            <IoMdAdd className='me-1' /> Add Niche
          </button>
        </div>
      </div>
