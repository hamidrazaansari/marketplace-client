...
function LanguageList({ handleRenderAddLang, onEdit }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resp = await axios.get('http://3.110.194.148:5000/api/languages');
  ...
  const handleDelete = async (id, lang) => {
    try {
      const result = await Swal.fire({
        title: `Are you sure to delete ${lang} ?`,
      ...
      if (result.isConfirmed) {
        ...
        await axios.delete(`http://3.110.194.148:5000/api/languages/${id}`);
        ...
        Swal.fire({
          title: "Deleted!",
          text: `Your ${lang} has been deleted.`,
          icon: "success"
        });
      }
    } catch (error) {
  ...
  const filteredLanguages = language.filter((item) =>
    item.langName.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const body = filteredLanguages.map((item) => {
    return {
      select: <input type="checkbox" onChange={(e) => handleSelectAll(e)} />,
      title: item.langName || "Unknown",
      ...
      action: (
        <>
          ...
          <button className='delete-btn' onClick={() => { handleDelete(item._id, item.langName); }}><RiDeleteBin6Line /></button>
        </>
      )
    };
  });
    ...
    <>
      <div className='d-flex align-items-center justify-content-between m-2 mb-2'>
        <div className='csv-heading'>
          <h3><IoMdArrowRoundBack className='me-1 mb-1' onClick={handleBack} />Language</h3>
        </div>
        <div>
          <button className='clear-btn me-4' onClick={handleRenderAddLang}>
            <IoMdAdd className='me-1' /> Add Language
          </button>
        </div>
      </div>
